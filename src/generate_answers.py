...
def generate_until(model, prefixes, stop_token, ml):
    ...
    for i, decoded_line in enumerate(decoded):
        ...
        # decoded_line = detokenizer.detokenize(decoded_line)
        ...
        # ----------
        # make sure we generated at least 1 sentence after the <ANSWER> tag.
        ...
        if len(decoded_line) == 1:
            ...
            decoded_line = re.split('<[A-Z]+>', decoded_line[0])
        if len(decoded_line) == 1:
            print(f"generated the <ANSWER> tag but no end-of-sentence in '{decoded_line[0]}'")
            decoded[i] = decoded_line[0] + ". . ."
...
def main():
    ...
    # strip out the answer but keep everything else
    all_test_lines = list(map(lambda line: line.split(' <ANSWER>')[0].strip() + ' <ANSWER>', all_test_lines))

    print("applying BPE and sorting lines by their lengths...")
    # apply BPE
    all_test_lines = bpe.apply(all_test_lines)
    # undo BPE on first names
    ...
    for bid, batch_lines in enumerate(all_test_lines):
        ...
        answers = generate_until(model, batch_lines, '<pad>', params.max_length)

        for i, line in enumerate(batch_lines):
            done_lines = np.append(
                done_lines, line.replace('<ANSWER>', '<ANSWER> ' + answers[i].strip())
            )
